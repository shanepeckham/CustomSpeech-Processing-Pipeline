...
TRANSCRIBED_FILE = '/Users/shanepeckham/sources/BTSports/File/11_WTA_ROM_STEPvGARC_2018/11_WTA_ROM_STEPvGARC_2018.txt'

# This is the file that is automatically generated by the Speech to Text process
AUDIO_PROCESSED_FILE = '/Users/shanepeckham/sources/BTSports/File/11_WTA_ROM_STEPvGARC_2018/11_WTA_ROM_STEPvGARC_OFFSET.txt'

# This is the automated transcript that will be generated
GENERATED_TRANSCRIPT_FILE = '/Users/shanepeckham/sources/BTSports/Results2/11_WTA_ROM_STEPvGARC_TRANSCRIPT_OUTPUT.txt'
...
def search_target(target_sentence, api_text, nlp_api_response):
    ...
    match_results.extend(list(map(compute_ratio, target_sentence2)))
    match_results.extend(list(map(compute_partial_ratio, target_sentence2)))
    fuzzy_results = process.extract(api_text, target_sentence2)
    match_results.extend(list(map(append_fuzzy_extract, fuzzy_results)))
    match_results.extend(list(map(compute_semantic_similarity, target_sentence2)))
    ...
    for row in df_final_results.itertuples():
        match_results.append(compute_sequencematcher(row.Text))
...
with codecs.open(GENERATED_TRANSCRIPT_FILE, "w", "utf-8-sig") as transcript:
    with codecs.open(AUDIO_PROCESSED_FILE, "r", "utf-8") as audio_processed_file:
        ...
        for line in audio_processed_file:
            ...
            if offset_word == 0:
                target_sentence2.append(target[:nlp_api_response_len].text)
            else:
                target_sentence2.append(target[offset_word:(nlp_api_response_len + offset_word) + 1].text)
                if offset_word - 1 > 0:
                    target_sentence2.append(target[(offset_word - 1):(nlp_api_response_len + offset_word)].text)

            match = search_target(target_sentence2, api_text, nlp_api_response)
            ...
            if match['Score'][match.index.values[0]] < (PASS2_THRESHOLD/100):
                ...
                i = 0
                target_sentence2 = [target[:nlp_api_response_len].text]
                while i < WORD_SLIDE:  # Let's slide 20 words either way
                    for j in range(WORD_PAD):  # Let's pad extra words on either side
                        target_sentence2.append(target[(offset_word - i):(nlp_api_response_len + offset_word + j)].text)
                        target_sentence2.append(target[(offset_word + i):(nlp_api_response_len + offset_word + j)].text)
                    i += 1
...
print("\nProcessed", str(len(score_results)), "files with translation accuracy of", str(np.mean(score_results)*100)+"%")
